...
- (void)eraseDataSwitchChanged:(id)sender
{
	if (_eraseDataSwitch.on) {
		...
		UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:KKPasscodeLockLocalizedString(@"cancle", @"") destructiveButtonTitle:KKPasscodeLockLocalizedString(@"Enable", @"") otherButtonTitles:nil];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
	...
	if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            if (_passcodeLockOn) {
                cell.textLabel.text = KKPasscodeLockLocalizedString(@"关闭密码锁", @"");
            } else {
                cell.textLabel.text = KKPasscodeLockLocalizedString(@"开启密码锁", @"");
            }
        } else {
            cell.textLabel.text = KKPasscodeLockLocalizedString(@"修改密码", @"");
