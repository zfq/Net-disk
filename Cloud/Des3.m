...
unsigned char hexkey[24] = {0x01, 0x02, 0x03, 0};

void print_block(unsigned char *bl)
{
	int i;
	for (i=0; i < 8; i++)
		putchar(bl[i]);
...
#endif
void make_key_pair(unsigned char hexkey[24],
                   unsigned long en_keyout[96],
                   unsigned long de_keyout[96])
{
	des3key(hexkey, EN0, en_keyout);
	des3key(hexkey, DE1, de_keyout);
}
...
{
	int len = strlen(in_buf) + 1;
	len = (len + 7) & ~7;
	char *obuf = NULL;
	char *ibuf = NULL;
	char  inblock[8];
	char  outblock[8];
	char *pb, *pi, *po, *pb1, *pb2;
	int   times, i;
	...
	while (times) {
		...
		des3(inblock, outblock, key_sched);
		pb1 = outblock;
		while (pb1 - outblock < 8)
			*po++ = *pb1++;
		times--;
	}
	free(ibuf);
